...
├── metadata.json                                     # The record — full session metadata, token counts, step index
├── commands/                                         # Derived views of metadata.json, written for reading
...
**`metadata.json` is the record; `commands/` is a projection of it.** The four command files are written per step for human reading and for replay, and they are never read back as a source of truth — a correction applied to metadata after the fact does not propagate into them, and they are not regenerated, because regenerating would overwrite a corrected value with the superseded one. Consumers should read `metadata.json`.
